    ...
    <div class="container pt-6 pb-28">
        <div class="bg-white rounded-fifteen p-25px relative mb-15px">
            ...
            <div class="flex gap-x-15px">
                ...
                <div class="flex gap-x-15px">
                    <button class="p-[9px] rounded-ten border transition-colors max-h-10"
                      @mouseover="isHoveredSort = true" @mouseleave="isHoveredSort = false" @click="sortToggleActive()"
                      :class="isHoveredSort ? 'border-zumthor text-dodger bg-zumthor' : 'text-slate-custom border-athens bg-athens-gray'"><svg-icon
                          name="sort-list" width="20" height="20" /></button>
                    <button class="p-[9px] rounded-ten border transition-colors max-h-10"
                      @mouseover="isHoveredFunnel = true" @mouseleave="isHoveredFunnel = false"
                      @click="funnelToggleActive()"
                      :class="isHoveredFunnel ? 'border-zumthor text-dodger bg-zumthor' : 'text-slate-custom border-athens bg-athens-gray'"><svg-icon
        ...
        <div class="table-container">
            ...
            <div class="table-body">
                <div v-for="item in paginatedCandidates" :key="item.id" class="table-row">
                    ...
                    <div class="px-2.5 text-sm font-normal text-space">{{ item.resume }}</div>
                    <div class="px-2.5 text-sm font-normal text-space">{{ item.vacancy }}</div>
                    <div class="px-2.5 text-sm font-normal text-space">{{ item.stage }}</div>
                </div>
            </div>
        </div>
...
.table-row {
    ...
    grid-template-columns:
        1.778% 26.667% 8% 17.778% 17.778% 23.556%;
    gap: 10px;
    padding: 26px 25px;
    align-items: center;
}
