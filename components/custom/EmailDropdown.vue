<template>
    <div class="relative">
        <!-- выбранный элемент -->
        <div class="flex items-center justify-between bg-athens-gray rounded-ten py-2 px-4 cursor-pointer"
          @click="toggleDropdown">
            <div class="text-sm font-medium flex items-center space-x-2">
                ...
                <span class="text-gray-700">{{ selectedOption?.email || placeholder }}</span>
            </div>
            <svg class="w-[18px] h-[18px] transform transition-transform" :class="{ 'rotate-180': isDropdownVisible }"
        ...
        <transition name="slide-fade">
            <ul v-if="isDropdownVisible" class="absolute bg-white border rounded-lg shadow-lg w-full mt-2 z-10">
                <li v-for="(option, index) in options" :key="index" @click="selectOption(option)"
                  class="flex items-center py-2 px-4 cursor-pointer hover:bg-gray-100">
                    <div class="flex gap-x-2 items-center max-w-[200px] w-full">
                        <img v-if="option.icon" :src="option.icon" alt="Icon" class="w-5 h-5" />
                        <p>{{ option.name }}</p>
                    </div>
                    <span class="text-gray-700 max-w-56 truncate">{{ option.email }}</span>
                </li>
            </ul>
        </transition>
    </div>
