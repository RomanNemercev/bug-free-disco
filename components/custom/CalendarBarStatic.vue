...
import { createDecade, createYear, toDate } from 'radix-vue/date' // импорт работает корректно. Его источник скорей всего разворачивается через рендер или является алиасом
  ...
  <CalendarRoot v-slot="{ date, grid, weekDays }" v-model:placeholder="placeholder" v-bind="forwarded"
    ...
    <CalendarHeader>
      <CalendarHeading class="flex w-full items-center justify-between gap-2">
        ...
        <Select :default-value="placeholder.year.toString()" @update:model-value="
          v => {
            if (!v || !placeholder) return
