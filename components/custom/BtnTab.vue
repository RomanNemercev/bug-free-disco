<template>
  <div class="gap-x-2.5 flex">
    <button v-for="tab in tabs" :key="tab.value"
      class="rounded-ten leading-normal py-11.5px px-15px text-sm font-medium transition-colors relative" :class="{
        'bg-space text-white': tab.value === modelValue,
        'bg-athens': hoveredTabs[tab.value],
      }" @click="selectTab(tab.value)" @mouseover="hoveredTabs[tab.value] = true"
      @mouseleave="hoveredTabs[tab.value] = false">
      {{ tab.label }}
      <span v-if="tab.notification"
